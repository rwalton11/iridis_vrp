...
set set_LEAVE={i in ALL_NODES, j in ALL_NODES, k in TECHS, t in HOURS: i!=j};
set set_ARRIVE={i in ALL_NODES, j in ALL_NODES, k in TECHS, t in HOURS: i!=j};
...
# ------------------------------------------------------------
# constraints relating to skill levels - added after preprocessing removed
# ------------------------------------------------------------

# techs can only be assigned to jobs that they are qualified to do

subject to c_skills{j in JOBS, k in TECHS}:
	sum{i in ALL_NODES, t in HOURS:(i,j,k,t) in set_LEAVE}var_leave[i,j,k,t] <=
		job_tech[j,k]*(1-var_twotech[j])+(sum{kp in TECHS}job_tech2[j,k,kp])*var_twotech[j];
...
# the total time spent in each job is equal to the duration of that job (by hours_tech variables)
